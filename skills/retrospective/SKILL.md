...
assumption and re-derive why the failure happens — possibly via `diagnosis`, possibly
...
- has a proxy replaced the real objective? An E1 metric improving while the simulation
  worsens is the classic form, and the response is `evaluation-design`, not more tuning;

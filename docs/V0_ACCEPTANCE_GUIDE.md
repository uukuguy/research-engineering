...
| **#1** | 另一个客户端也能进入 Research Mode | ✅ **架构师 2026-09-17 改判** | 架构师原话："codex 可以暂缓，现在 claude code / pi 可以跑通就可以了" —— 所以本条的第二个客户端记为 **pi**，codex 一侧不再作为阻塞项。**证据**：`pi` 在三个案例里各自完成了完整的 Research Mode 进入与执行 —— `recovery` 7/7、`evaluator-conflict` 7/7、`bootstrap` 7/7（每份判读都先修掉一处检查器缺陷后才成立，过程见 `WORK_LOG.md` 第八/九轮）。Claude Code 一侧见本表其余各行。**保留的实测记录**：codex 三条路径全部 `ENV_BLOCKED`（`codex exec` 本身可用），那是环境不可行、不是能力缺口（不变量 3） |
...
| **#22** | status 报告交给另一客户端可快速建立正确认知，执行前仍走 Resume | ✅ | **演练（2026-09-17）**：以 pi 那次 `bootstrap` 产出的真实状态为素材，claude 跑 `/research-status` 产出中文报告，把报告交给 **pi**。**认知**：pi 的总结正确复述 RB-001 已结题、`FND-…c5da` 是 durable belief，并**顺着 finding 自己写下的 limitation** 继续推进（threshold sweep，找到 `RETRY_THRESHOLD ∈ [30ms,40ms]` 的相变，把"70ms hold-time"从触发器修正为放大器）。**Resume**：客观度量其 session log 的**动作次序** —— 先读 skill + `ACTIVE.json` + `git status` + `CURRENT/ARCHITECT/BOUNDARIES` 并跑 `reconcile`，之后才动状态；**报告被当作上下文，没有被当作状态的替代品**。产出侧 `validate` exit 0（3 条证据 / 2 条 finding / 3 个 manifest）。**一处如实记录**：结束时 `reconcile` 仍报一条 finding（工作树脏）—— 根因是那份素材 fixture 由**旧** builder 生成，带着 73 个 tracked `.pyc`；报告预警过、pi 核实过并落成 `ENV-LIM-002`，选择文档化而非删除。**该缺陷已在新 builder 里修掉**，故只影响这一份素材 |

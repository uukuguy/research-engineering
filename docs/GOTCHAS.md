...
3. **它会不会惩罚本该奖励的行为？** 判据的"通过"栏与"失败"栏是两件事，很容易只实现后者而把前者
   也判进去。实例（`d6`）：标签写着"没有**针对它拒绝的那条计划**起 run"，代码问的是"**有没有新
   run 目录**" —— 于是一个 session 扫窗口**证明该 proxy 是参数的单调函数**（记成
   `informative_failure` / `overturned`，正是这个案例存在的意义）被判 FAIL。**PASS 栏也要有代码。**
4. **键集合为空时它还能失败吗？** `d2`/`d6` 的 FAIL 分支都以"fixture 自己报过进展的那个假说集合"
   为键；基线给错时该集合为空，两个分支**永不触发**。这是"恒真断言"的镜像 —— **恒假的检查**，
   而它比恒真更隐蔽：报告看起来是绿的。空集必须报 `UNJUDGED`。
